    ...
    pub fn transformer(input: Vec<(String, Command)>) -> Vec<String> {
        ...
        for (string, command) in input.iter() {
            ...
            match command {
                ...
                Command::Append(number) => output.push(string.clone() + &"bar".repeat(number.clone())),
            }
        }
        output

    //     input.iter()
    //     .map(|(string, command)| -> String {
    //         match command {
    //             Command::Uppercase => string.to_uppercase().to_string(),
    //             Command::Trim => string.trim().to_string(),
    //             Command::Append(number) => string.clone() + &"bar".repeat(number.clone()),
    //         }
    //     }).collect()
    }
}
